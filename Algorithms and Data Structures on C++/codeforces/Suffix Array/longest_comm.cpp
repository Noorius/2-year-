...
int main() {
    ...
    cin>>s>>t;
    string conc = s+"+"+t;
    auto result = computeSuffixArray(conc);
    ...
    cout << endl;
    */
    vector<int> lcp = computeLCP(result.first, result.second, conc);

    /*for (int i = 2; i < result.first.size(); i++) {
        cout<<lcp[i]<<" "<<conc.substr(result.first[i], result.first.size()-result.first[i])<<endl;
    }*/
