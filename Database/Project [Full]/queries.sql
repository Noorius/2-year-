...
SELECT b.BIN, to_char(ord.date,'Month') as month, COUNT(DISTINCT ord.ORD_IN) as trend
FROM orders ord,vehicle v,models mod,brands b
WHERE ord.VIN=v.VIN AND
      v.MOD_IN = mod.MOD_IN AND
      b.BIN = mod.BIN AND
      ord.date >= now()-interval'3 year'
GROUP BY month,b.BIN
ORDER BY b.BIN ASC, trend DESC;
...
SELECT b.BIN, EXTRACT(YEAR FROM ord.date) as year, r_from, r_to, COUNT(DISTINCT ord.ORD_IN) as trend
...
GROUP BY b.BIN, year, r_from, r_to
ORDER BY b.BIN ASC, year ASC, r_from ASC;
...
WHERE v.VIN=op.VIN AND
      ...
      p.manufactured_date BETWEEN '2020-01-09' AND '2021-01-21';
...
WHERE ord.VIN=v.VIN AND
      v.MOD_IN = mod.MOD_IN AND
      b.BIN = mod.BIN AND
      ord.date >= now()-interval'1 year' AND
      ord.oper_type = 'Purchase'
GROUP BY b.BIN
...
SELECT to_char(ord.date,'Month') as month, count(DISTINCT ord.ORD_IN)
