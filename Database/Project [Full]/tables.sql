...
COPY words(word)
FROM 'C:\\Users\\Public\\1000-words-csv.csv' WHERE word NOT LIKE '%''%';

CREATE TEMPORARY TABLE city(city TEXT);
COPY city(city)
FROM 'C:\\Users\\Public\\cities.csv' WHERE city NOT LIKE '%''%';

INSERT INTO address(zip, street, city)
SELECT
(random() * 99998 + 1),
(SELECT initcap(word) FROM words ORDER BY random()+_g*0 LIMIT 1),
(SELECT city FROM city ORDER BY random()+_g*0 LIMIT 1)
FROM generate_series(1, 1000) AS _g
...
INSERT INTO final_assembly_factory VALUES (2, 'Conveer');
INSERT INTO final_assembly_factory VALUES (3, 'Conveer');
...
INSERT INTO part(part_in, error, manufactured_date, supp_in, fact_in)
SELECT
(random()*1000)::integer,
FALSE,
...
9,
NULL
FROM generate_series(1, 100) AS _g
ON CONFLICT DO NOTHING;
...
INSERT INTO vehicle(VIN, manufactured, country, certified, PARK_IN, MOD_IN)
SELECT
(random()*1000)::integer, --VIN
(now() - '5 years'::interval * random())::date, --manufactured (date)
'Kazakhstan', --country
True, --certified
(SELECT PARK_IN FROM inventory ORDER BY random()+_g*0 LIMIT 1), --FK PARK_IN
(SELECT MOD_IN FROM models ORDER BY random()+_g*0 LIMIT 1) --FK MOD_IN
FROM generate_series(1, 100) AS _g --DO it 100 times
ON CONFLICT DO NOTHING; --IF error, then skip
SELECT * FROM vehicle;

CREATE TABLE options(
    VIN varchar(30) PRIMARY KEY,
    colours varchar(30) NOT NULL,
    engine varchar(30) NOT NULL UNIQUE,
    transmission varchar(30) NOT NULL UNIQUE,
...
DECLARE
    q record;
    arr TEXT[] := array['White','Blue','Orange','Violet','Gray','Black','Yellow','Pink','Purple','Brown'];
BEGIN
    FOR q in (SELECT VIN FROM vehicle) LOOP
        INSERT INTO options(VIN, colours, engine, transmission)
        SELECT
        q.VIN,
        (arr[floor(random()*9+1)]),
        (SELECT PART_IN FROM part ORDER BY random()+_g*0 LIMIT 1),
        (SELECT PART_IN FROM part ORDER BY random()+_g*0 LIMIT 1)
        FROM generate_series(1, 1) AS _g
        ON CONFLICT DO NOTHING;
    end loop;
end;
$$;
...
DECLARE
    q record;
BEGIN
    FOR q in (SELECT * FROM vehicle) LOOP
        INSERT INTO orders(oper_type, amount, date, DEAL_IN, CUST_IN, VIN)
        SELECT
        'Purchase',
        (random()*100000)::integer,
        (now() - '5 years'::interval * random())::date,
        (SELECT DEAL_IN FROM dealers ORDER BY random() LIMIT 1),
        (SELECT IIN FROM customers ORDER BY random() LIMIT 1),
        q.VIN
        ON CONFLICT DO NOTHING;
    end loop;
end;
